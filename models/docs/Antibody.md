...
[![img](https://yuml.me/diagram/nofunky;dir:TB/class/[SequenceReference],[RiskGroup],[Provider],[ProductCategory],[Product],[PathogenIdentification],[Originator],[MSDS],[Keyword],[Image],[IATAClassification],[Hybridoma],[ExternalRelatedReference],[Document],[DOI],[ContactPoint],[Collection],[Certification],[SequenceReference]<sequenceReference%200..*-++[Antibody&#124;productionSystem:string%20%3F;antibodyPurifiedByAffinity:boolean;specificityDocumented:boolean;targetedAntigen:string;shippingConditions(i):string;storageConditions(i):string;thirdPartyDistributionConsent(i):boolean%20%3F;usageRestrictions(i):string%20%3F;accessPointURL(i):uri;refSKU(i):string;unitDefinition(i):string%20%3F;unitCost(i):string;qualityGrading(i):string%20%3F;biosafetyRestrictions(i):string%20%3F;canItBeUsedToProduceGMO(i):boolean%20%3F;availability(i):string;technicalRecommendation(i):string%20%3F;internalReference(i):string%20%3F;note(i):string%20%3F;name(i):string;description(i):string%20%3F],[Antibody]^-[Hybridoma],[Product]^-[Antibody])](https://yuml.me/diagram/nofunky;dir:TB/class/[SequenceReference],[RiskGroup],[Provider],[ProductCategory],[Product],[PathogenIdentification],[Originator],[MSDS],[Keyword],[Image],[IATAClassification],[Hybridoma],[ExternalRelatedReference],[Document],[DOI],[ContactPoint],[Collection],[Certification],[SequenceReference]<sequenceReference%200..*-++[Antibody&#124;productionSystem:string%20%3F;antibodyPurifiedByAffinity:boolean;specificityDocumented:boolean;targetedAntigen:string;shippingConditions(i):string;storageConditions(i):string;thirdPartyDistributionConsent(i):boolean%20%3F;usageRestrictions(i):string%20%3F;accessPointURL(i):uri;refSKU(i):string;unitDefinition(i):string%20%3F;unitCost(i):string;qualityGrading(i):string%20%3F;biosafetyRestrictions(i):string%20%3F;canItBeUsedToProduceGMO(i):boolean%20%3F;availability(i):string;technicalRecommendation(i):string%20%3F;internalReference(i):string%20%3F;note(i):string%20%3F;name(i):string;description(i):string%20%3F],[Antibody]^-[Hybridoma],[Product]^-[Antibody])
